...
describe('Proposal class', () => {
  test('Proposal::submitFor', () => {
    ...
    expect((proposal.applied[0] as ProposalSubmitted).client.id.value).toBe(
      client.id.value
    )
  ...
  test('Proposal::rejectPricing', () => {
    ...
    expect((proposal.applied[0] as PricingRejected).proposalId.value).toBe(
      uniqueId.value
    )
    expect((proposal.applied[0] as PricingRejected).suggestedPricing).toBe(
      fairPrice
    )
  ...
  test('Proposal::poolDoers', () => {
    ...
    expect((proposal.applied[0] as DoersPooled).proposalId.value).toBe(
      uniqueId.value
    )
    expect(
      (proposal.applied[0] as DoersPooled).doers.contains(doer1)
    ).toBeTruthy()
    expect(
      (proposal.applied[0] as DoersPooled).doers.contains(doer2)
    ).toBeTruthy()
    expect(
      (proposal.applied[0] as DoersPooled).doers.contains(doer3)
    ).toBeTruthy()
  ...
  test('Proposal::mergeDoers', () => {
    ...
    expect(
      (proposal.applied[0] as DoersMerged).doers.contains(doer1)
    ).toBeTruthy()
    expect(
      (proposal.applied[0] as DoersMerged).doers.contains(doer2)
    ).toBeFalsy()
    expect(
      (proposal.applied[0] as DoersMerged).doers.contains(doer3)
    ).toBeTruthy()
